...
test('remote driver sends only session handle and semantic operations',async()=>{
  const calls=[];
  const factory=createRemoteDriverFactory({endpoint:'https://worker.example.test',fetch:async(url,init)=>{calls.push([url,JSON.parse(init.body)]);return {ok:true,json:async()=>calls.at(-1)[1].op==='DETECT_AUTHORIZATION_DENIAL'?{ok:true,denial:null}:{ok:true,tracking_id:'T1',received_at:'2026-09-16T12:00:00Z',confirmation_url:'https://r'}};}});
  const driver=await factory({authority_id:'CA_CDT_PRA',session_handle:'sess-1',fingerprint:'fp'});
  await driver.open('https://portal.example.test'); const denial=await driver.detectAuthorizationDenial(); await driver.fill('subject','Records'); const receipt=await driver.submit({request_id:'R1',fingerprint:'fp'});
